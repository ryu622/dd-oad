...
### 再現性チェック(seed=7、別50イベントサブサンプル)

上記(c)を検証するため、`fit_pooled_real_subsample.py`に外部からシード・出力先を指定できる機能を追加し(`sys.argv`経由、デフォルトは従来通りseed=42)、独立な別サブサンプル(seed=7、26.4分で完了)で再実行した。

| 共有パラメータ | seed=42 | seed=7 |
|---|---|---|
| $f_{d0}$ | 6.709 | **11.452**(探索上限12.0にかなり接近) |
| $f_{d,\infty}$ | 4.271 | 4.230 |
| $d_0$ | 1.181 | 2.946 |
| 平均誤差(中央値) | 2.169(1.155) | 2.637(1.570) |
| 方向($f_{d0}>f_{d,\infty}$) | ✓ | ✓ |

- **方向は2つの独立サブサンプルで再現した**。H1'にとってプラスの材料。
- **$f_{d,\infty}$は非常に安定**(4.27 vs 4.23) — 「遠距離での駆動力」はサブサンプルに依らず一貫した値。
- 一方で**$f_{d0}$と$d_0$はサブサンプル間で大きくぶれている**($f_{d0}$は6.7→11.5、$d_0$は1.18→2.95)。特に$f_{d0}=11.45$は探索上限(12.0)にかなり接近しており境界張り付きに近い挙動で、「近距離での駆動力」自体は50件規模ではまだ安定推定できていない可能性が高い。フィット精度もseed=7の方が悪化(誤差≥2が21/50)。

**更新した結論**: 「近づくと駆動力が強まる」という方向性の主張は2回の独立試行で支持されたが、定量的な大きさ($f_{d0}$, $d_0$)は不安定で、50件では標本サイズが不足している可能性が高い。次のステップは全224件(またはより大きなサブサンプル)での再フィットにより、この不安定さが収束するか確認すること。
